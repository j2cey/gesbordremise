...
class WorkflowAction extends BaseModel
{
    ...
    public function fieldsrequiredwithout() {
        return $this->belongsToMany('App\WorkflowObjectField', 'fields_required_without', 'workflow_action_id', 'workflow_object_field_id')
            ->withTimestamps()
            ;
    }
    ...
    public function fieldsrequiredwith() {
        return $this->belongsToMany('App\WorkflowObjectField', 'fields_required_with', 'workflow_action_id', 'workflow_object_field_id')
            ->withTimestamps()
            ;
    }
    ...
    public function Traiter(WorkflowExecModelStep $workflowexecmodelstep, $request, $images_dir) {
        ...
        if ($this->type->code === "2") {
            // action sur objet
            $model = $workflowexecmodelstep->model_type::where('id', $workflowexecmodelstep->model_id)->first();
